...
__all__ = ['ds_swin_base_patch4_window7_224', 'ds_vit_base_patch16_224', 'ds_deit_base_patch16_224', 'ds_vit_small_patch16_224','ds_vit_large_patch16_224','ds_deit_large_patch16_224']
...
class Vit(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',init_values=None,no_embed_class=False, **kwargs):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,init_values=init_values)
...
class PMTrans(nn.Module):
    arch_zoo = ['swin_base', 'vit_base', 'deit_base','vit_small','vit_large','deit_large']
    ...
    def __init__(self, num_classes=65, model_name='swin_base'):
        ...
        if isinstance(model_name, str):
            ...
            if model_name == 'swin_base':
                ...
                model_name = 'ds_vit_small_patch16_224'
            elif model_name == 'vit_large':
                model_name = 'ds_vit_large_patch16_224'
            elif  model_name == 'deit_large':
                model_name = 'ds_deit_large_patch16_224'
    ...
    def forward(self, target, infer_label, source=None, mem_fea=None, mem_cls=None, class_weight_src=None,
        ...
        t_logits, t_p, t_attn = self.backbone.forward_features(t_token, patch=True) #t_logits : target class token, t_p target: all token, B*token_num*embd_dim
        ...
        if self.training:
            ...
            dis = -torch.mm(t_logits.detach(), mem_fea.t()) #16*4935 distance between each target sample and memory in latent feature space(class token)
            for di in range(dis.size(0)):
                #set the distance between the same sample in batch and memory to be the maximum
                dis[di, img_idx[di]] = torch.max(dis)   #16 img_idx: the index of image in memory(all target samples)
            _, p1 = torch.sort(dis, dim=1) #16*4935 sort the distance between each sample in batch and samples in memory(target class token)
            w = torch.zeros(t_logits.size(0), mem_fea.size(0)).cuda() # 16*5000

            for wi in range(w.size(0)):  #every sample
                for wj in range(5):  # 5 nearest neighbor
                    w[wi][p1[wi, wj]] = 1 / 5 # the weights is 1/5 in the 5 nearest neighbors location
            #the probablities of target belongint to the most probable class and its predition label
            weight_tgt, pred = torch.max(w.mm(mem_cls), 1) #mem_cls: the class label vector of memory samples
...
@register_model
def ds_vit_large_patch16_224(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    ...
    model = Vit(**model_kwargs)
    if pretrained:
        pre = create_model('vit_large_patch16_224', pretrained=pretrained)
        model.load_state_dict(pre.state_dict())
    return model

@register_model
def ds_vit_small_patch16_224(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    ...
    model = Vit(**model_kwargs)
    if pretrained:
        pre = create_model('vit_small_patch16_224', pretrained=pretrained)
        model.load_state_dict(pre.state_dict())
    return model
...
@register_model
def ds_deit_large_patch16_224(pretrained=False, **kwargs):
    """ DeiT-base distilled model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16,no_embed_class=True, init_values=1e-6, **kwargs)
    model = Vit(**model_kwargs)
    if pretrained:
        pre = create_model('deit3_large_patch16_224', pretrained=pretrained)
        model.load_state_dict(pre.state_dict())
    return model
